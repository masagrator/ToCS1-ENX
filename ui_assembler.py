...
def GenerateCommand(entry, Offset_dict = None):
	...
	match(entry["TYPE"]):
		...
		case "0x13":
			...
			elif (entry["CONTROL"] in [0xB, 0x22, 0x2B, 0x38, 0x3B]):
				ret_entry.append(entry["STRINGS"][0].encode("UTF-8") + b"\x00")
			if (entry["CONTROL"] in [0xB, 0x22, 0x2B]):
...
files = glob.glob("scripts/ui/jsons/*.json")

os.makedirs("scripts/ui/nx", exist_ok=True)
for i in range(0, len(files)):
	...
	# Write Header
	file_new = open("scripts/ui/nx/%s.dat" % files[i][17:-5], "wb")
